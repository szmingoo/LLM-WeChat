...
def get_url():
    url = url_entry.get()  # 获取输入框的内容
    if url:
        # 在这里执行你的程序逻辑，例如使用 url 变量
        import os
        import random
        import string
        import asyncio
        ...
        messagebox.showwarning("警告", "请输入一个有效的 URL")
...
# 创建输入框和按钮
url_label = tk.Label(root, text="请输入公众号软文地址:")
url_label.pack(pady=5)

url_entry = tk.Entry(root, width=50)
url_entry.pack(pady=5)

get_button = tk.Button(root, text="获取", command=get_url)
